...
CACHED_TARBALL="${BUILD_DIR}/downloads/zig-0.13.0-x86_64-linux.tar.xz"
...
cp "${ZIG_RELEASE_TARBALL}" "${CACHED_TARBALL}"
...
if ! grep -Fq '"llvm_ref":"llvmorg-20.1.8"' "${STATE_DIR}/zig-source-lock.json"; then
	echo "expected cached release inference to select latest llvm patch"
